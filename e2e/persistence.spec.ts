...
test("resumes a saved civilization after reload and supports clearing it", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Pause" }).click({ timeout: 10_000 });
  await page.getByRole("button", { name: "Reset" }).click({ timeout: 10_000 });
  await expect(generationBadge).toHaveText("Generation 0");

  await page.getByRole("button", { name: "Epoch" }).click({ timeout: 10_000 });
  ...
  await page.getByRole("button", { name: "Clear save" }).click({ timeout: 10_000 });
